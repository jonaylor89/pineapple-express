...
export const actions = {
  async fetchVisitedUserByUsername(ctx, username) {
    ...
    const userQuery = await this.$fire.firestore
      .collection("users")
  ...
  async fetchUserLoop(ctx, visitedUserId) {
    const userLoopsQuery = await this.$fire.firestore
      .collection("loops")
    ...
    const userLoopsFiltered = userLoops.filter((loop) => loop.deleted !== true);

    ctx.commit("SET_USER_LOOPS", userLoopsFiltered);
  },
  async fetchFollowerCount(ctx, visitedUserId) {
    const followersSnapshot = await this.$fire.firestore
  ...
  async fetchFollowingCount(ctx, visitedUserId) {
    const followingsSnapshot = await this.$fire.firestore
  ...
  async fetchIsFollowing(ctx, visitedUserId) {
    try {
      ...
      const isFollowingDoc = await this.$fire.firestore
        .collection("followers")
        .doc(visitedUserId)
        .collection("Followers")
        .doc(currentUserId)
